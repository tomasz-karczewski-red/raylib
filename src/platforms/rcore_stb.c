...
#define KHRONOS_APIENTRY // TODO: why? will fail at egl headers without it
// #include <EGL/egl.h>
// #include <GLES2/gl2.h>
...
int InitPlatform(void)
{
    ...
    platform.device = eglGetDisplay(display);
    /////////////////////////////////
    ...
    if (!eglChooseConfig(platform.device, framebufferAttribs, &platform.config, 1, &numConfigs)) {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to eglChooseConfig");
        return -1;
    }

    // Set rendering API
    //  TODO: why ? eglBindAPI(EGL_OPENGL_ES_API);
...
#ifdef THIS_IS_NOT_ANDROID_IS_IT    
    ...
    if (!window->surface) {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to create wayland surface");
        return -1;
    }
    window->shell_surface = wl_shell_get_shell_surface(shell, window->surface);
    if (!window->shell_surface) {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to create shell_surface");
        return -1;
    }
    wl_shell_surface_add_listener(window->shell_surface,
            &shell_surface_listener, window);
    wl_shell_surface_set_toplevel(window->shell_surface);
    fprintf(stderr, "xaxa CORE.Window.display.width: %d, CORE.Window.display.height: %d\n", CORE.Window.display.width, CORE.Window.display.height);
    window->egl_window = wl_egl_window_create(window->surface, CORE.Window.screen.width,CORE.Window.screen.height); // CORE.Window.display.width, CORE.Window.display.height);
    if (!window->egl_window) {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to create egl_window");
        return -1;
    }
    platform.surface = eglCreateWindowSurface(platform.device, platform.config, window->egl_window, NULL);
    if (EGL_NO_SURFACE == platform.surface) {
        TRACELOG(LOG_WARNING, "DISPLAY: Failed to create egl egl_window");
        return -1;
    }
    // eglMakeCurrent(platform.device, platform.surface, platform.surface,
    //         platform.context);
#endif
